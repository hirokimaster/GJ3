...
ConstantBuffer<BloomDissolveParam> gBloomDissolve : register(b0);
...
float32_t4 WeightCalc(float32_t2 texCoord)
{
    ...
    float32_t sigma = gBloomDissolve.sigma;
    float32_t stepWidth = gBloomDissolve.stepWidth;
...
float32_t4 ExtractLuminance(float32_t4 col)
{
    ...
    float32_t extract = smoothstep(gBloomDissolve.bloomThreshold, gBloomDissolve.bloomThreshold + 0.3, grayScale);
    return col * extract;
}
...
PixelShaderOutput main(VertexShaderOutput input)
{
    ...
    // 元の色と高輝度部分を合成する
    float32_t4 LuminanceGaussian = LuminanceColor * gBloomDissolve.lightStrength + normalColor;

    float32_t mask = gMaskTexture.Sample(gSampler, input.texcoord);
    // maskの値0.5(閾値)以下の場合はdiscardして抜く
    if (mask <= gBloomDissolve.threshold)
    {
        discard;
    }
